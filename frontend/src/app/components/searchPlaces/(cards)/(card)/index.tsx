...
export default function Card({ place }: Props) {
  ...
  const addTouristSpot = async () => {
    if (!token) {
      setStatusMessage("Você precisa estar logado para adicionar um local.");
      return null;
    }
    ...
    try {
      ...
      if (touristSpotResponse.status === 400) {
        setStatusMessage("O local já existe.");
        return null;
      }

      if (touristSpotResponse.status !== 201) {
        setStatusMessage("Não foi possível adicionar o local.");
        return null;
      }
  ...
  const addToFavorites = async () => {
    ...
    try {
      ...
      if (favoriteResponse.status === 204 || favoriteResponse.status === 201) {
        setStatusMessage("Adicionado aos favoritos com sucesso!");
        return favoriteResponse.status;
      } else {
        setStatusMessage("Falha ao adicionar aos favoritos.");
        return null;
      }
    } catch (error: any) {
      handleError(error);
